...
class TestPPOLoss:
    ...
    def test_policy_loss_clipped_for_high_logprobs(self, loss_fn):
        ...
        # ratio will be [e, e, e]
        # clipped ratio becomes [1.2, 1.2, 1.2]
        # objective becomes max(-e, -1.2) since advantages is 1
        expected_loss = torch.tensor(-1.2)

        _, policy_loss, _ = loss_fn(pi_old_logprobs, pi_logprobs_high, advantages, values, returns)
    ...
    def test_policy_loss_clipped_for_low_logprobs(self, loss_fn):
        ...
        # clipped ratio becomes [0.8, 0.8, 0.8]
        # objective becomes max(1/e, 0.8) since advantages is 1
        expected_loss = torch.tensor(0.8)

        _, policy_loss, _ = loss_fn(pi_old_logprobs, pi_logprobs_low, advantages, values, returns)
    ...
    def test_policy_loss_not_clipped(self, loss_fn):
        ...
        _, policy_loss, _ = loss_fn(pi_old_logprobs, pi_logprobs_unclipped, advantages, values, returns)
    ...
    def test_policy_loss_lower_for_higher_advantages(self, loss_fn):
        ...
        _, policy_loss_low, _ = loss_fn(pi_logprobs, pi_logprobs, advantages_high, values, returns)
        _, policy_loss_high, _ = loss_fn(pi_logprobs, pi_logprobs, advantages_low, values, returns)
    ...
    def test_value_loss_lower_for_values_similar_to_return(self, loss_fn):
        ...
        _, _, value_loss_lower = loss_fn(pi_logprobs, pi_logprobs, advantages, values_similar, returns)
        _, _, value_loss_higher = loss_fn(pi_logprobs, pi_logprobs, advantages, values_less_similar, returns)
