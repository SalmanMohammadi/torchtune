...
def truncate_sequence_at_first_stop_token(
    ...
    Returns:
        tuple[torch.Tensor, torch.Tensor]: tuple of two tensors:
            - eos_mask (torch.Tensor): tensor with the same shape as `sequences`
                where True indicates the corresponding token has been truncated
    ...
    eos_mask = (seq_lens > 1) | ((seq_lens == 1) & ~eos_mask)
    sequences[eos_mask] = fill_value
    return eos_mask, sequences
